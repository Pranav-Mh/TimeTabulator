...
router.get('/subjects/:year/:division', async (req, res) => {
  try {
    ...
    // Get practical subjects (PR) for this academic year
    ...
    // Combine subjects with their batch assignments
    const subjectsWithAssignments = subjects.map(subject => {
      const batches = [1, 2, 3].map(batchNum => {
        const assignment = existingAssignments.find(
          a => a.subjectId.toString() === subject._id.toString() && a.batchNumber === batchNum
        );
...
// Assign teacher to lab subject for specific division and batch
  ...
  try {
    ...
    if (!subjectId || !divisionId || !batchNumber || !teacherId) {
      return res.status(400).json({ 
        error: "Missing required fields: subjectId, divisionId, batchNumber, teacherId" 
      });
    }
    ...
    const lectureAssignments = await require('../models/LectureAssignment').find({ teacherId });
